...
makePicture p = Pictures [
    ...
    Translate 230 0 $ plotFeatures ("0 - id - "++ show idScaling) "0 - compilationRate - 1" (\fv -> (fromIntegral $ GRPPool.id fv) / idScaling) (fromRational . compilationRate) features,
    Translate 340 0 $ plotFeatures ("0 - id - "++ show idScaling) "0 - fitness - 1" (\fv -> (fromIntegral $ GRPPool.id fv) / idScaling) (fitness) features,
    Translate 450 0 $ plotFeatures ("0 - id - "++ show idScaling) "0 - children - 100" (\fv -> (fromIntegral $ GRPPool.id fv) / idScaling) (\fv -> ((1/100) * (fromIntegral $ GRPPool.children fv))) features,
...
cross :: Float -> Picture
cross axis = Pictures [Line [(-axis/2, -axis/2), (axis/2, axis/2)], Line [(axis/2, -axis/2), (-axis/2, axis/2)]]
...
plotFeatures lb1 lb2 ft1 ft2 fs = Pictures (map plotFeaturesSingle fs ++ axes ++ labels)
  where
    plotFeaturesSingle (Just fv) = Color (if GRPPool.id fv == 0 then green else if isLocalMax fv == LocalMax then red else if isLocalMax fv == Inherited then blue else  black) $ Translate (100 * (min 1 $ max 0 $ ft1 fv)) (100 * (min 1 $ max 0 $ ft2 fv)) $ (if GRPPool.children fv > 50 then Circle else cross)  (0.0 + activeRegression fv)
